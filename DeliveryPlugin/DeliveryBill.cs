...
{
    public class DeliveryBill : IPlugin
    {
        MyService myService;
        ...
        public void UpdateRequestDeliveryProduct_QuantityOnCreate(string LogicalName, Guid RequestDeliveryProductId, decimal Bill_Quantity)
        {
            ...
            decimal NetQuantity = (decimal)RequestDeliveryProduct["bsd_netquantity"] + Bill_Quantity;
            decimal RemainingQuantity = RequestQuantity - NetQuantity;
            Entity new_requestdeliveryproduct = new Entity(LogicalName, RequestDeliveryProductId);
            new_requestdeliveryproduct["bsd_netquantity"] = NetQuantity;
            new_requestdeliveryproduct["bsd_remainingquantity"] = RemainingQuantity;
            myService.service.Update(new_requestdeliveryproduct);
        }
    }
    public class GoodsIssueNote_Manager
    {
        private IOrganizationService service;
        public GoodsIssueNote_Manager(IOrganizationService service)
        {
            this.service = service;
        }
        /// <summary>
        /// update khi luwu phieesu xuat kho
        /// </summary>
        /// <param name="request"></param>
        /// <param name="deliveryScheduleId"></param>
        /// <param name="productId"></param>
        /// <param name="pre_quantity"></param>
        /// <param name="new_quantity"></param>
        public void UpdateDeliveryScheduleProduct(Entity request, Guid deliveryScheduleId, Guid productId, decimal pre_quantity, decimal new_quantity)
        {
            Entity deliveryScheduleProduct = service.RetrieveMultiple(new FetchExpression(@"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
              <entity name='bsd_deliveryplanproduct'>
                ...
                <order attribute='bsd_name' descending='false' />
                <filter type='and'>
                  <condition attribute='bsd_deliveryplan' operator='eq' uitype='bsd_deliveryplan' value='" + deliveryScheduleId + @"' />
                  <condition attribute='bsd_product' operator='eq' uitype='product' value='" + productId + @"' />
                </filter>
              </entity>
            </fetch>")).Entities.FirstOrDefault();

            decimal changeQuantity = new_quantity - pre_quantity;
            Entity updateProduct = new Entity(deliveryScheduleProduct.LogicalName, deliveryScheduleProduct.Id);
            decimal old_remainingquantity = (decimal)deliveryScheduleProduct["bsd_remainingquantity"];
            decimal new_remainingquantity = old_remainingquantity - changeQuantity;
            updateProduct["bsd_remainingquantity"] = new_remainingquantity;
            service.Update(updateProduct);

            this.UpdateDeliveryTruckOnGoodsIssueNoteProductChange(request, deliveryScheduleId, productId, new_quantity);
        }
        public void UpdateDeliveryTruckOnGoodsIssueNoteProductChange(Entity request, Guid deliveryScheduleId, Guid productId, decimal new_quantity)
        {
            // lấy thằng truck mà đã đc dùng để tạo ra request này, rồi tạo phiếu xuất kho này. có thêm đk sp.
            EntityCollection list_truck = service.RetrieveMultiple(new FetchExpression(@"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='true'>
              <entity name='bsd_deliveryplantruck'>
                ...
                <link-entity name='bsd_requestdeliverydeliveryplantruck' from='bsd_deliveryplantruck' to='bsd_deliveryplantruckid' alias='ae'>
                  <link-entity name='bsd_requestdelivery' from='bsd_requestdeliveryid' to='bsd_requestdelivery' alias='af'>
                    <filter type='and'>
                      <condition attribute='bsd_requestdeliveryid' operator='eq' uitype='bsd_requestdelivery' value='" + request.Id + @"' />
                    </filter>
                  </link-entity>
                </link-entity>
              </entity>
            ...
            if (list_truck.Entities.Any())
            {
                var truck = list_truck.Entities.First();
                decimal quantity = (decimal)truck["bsd_quantity"];
                Entity update_truck = new Entity(truck.LogicalName, truck.Id);
                update_truck["bsd_goodsissuenotequantity"] = new_quantity;
                update_truck["bsd_remaininggoodsissuenotequantity"] = quantity - new_quantity;
                service.Update(update_truck);
            }
        }
        /// <summary>
        /// laasy sos luong chua xuat kho cua san pham trong delivery schedule.
        /// </summary>
        /// <param name="productId"></param>
        /// <param name="deliverySchedule"></param>
        /// <returns></returns>
        public decimal GetRemainingQuantitybyProduct(Guid productId, Guid deliveryScheduleId)
        {
            Entity DeliveryScheduleProduct = service.RetrieveMultiple(new FetchExpression(@"
            ...
            if (DeliveryScheduleProduct == null) "Delivery Schedule Product Does Exist".Throw();

            decimal Total_Quantity = (decimal)DeliveryScheduleProduct["bsd_shipquantity"];

            // đã tạo.
            EntityCollection ProductListGoodsIssueNote = service.RetrieveMultiple(new FetchExpression(
             @"<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>  
                <entity name='bsd_deliveryproductbill'>    
                    <attribute name='bsd_deliveryproductbillid' />    
                    <attribute name='bsd_netquantity' />    
            ...
            decimal GoodsIssueNote_Quantity = 0m;
            if (ProductListGoodsIssueNote.Entities.Any())
            {
                foreach (Entity item in ProductListGoodsIssueNote.Entities)
                {
                    GoodsIssueNote_Quantity += (decimal)item["bsd_netquantity"];
                }
            }
            return Total_Quantity - GoodsIssueNote_Quantity;
        }
    }
}
